...
if wandb_upload:
    train_dataset = PairDataset(
        data_path, ma_list, re_list, aoa_list, "train", n_slices
    )
    test_dataset = PairDataset(
        data_path, ma_list, [4e6, 7e6, 1e7], aoa_list, "test", n_slices
    )
else:
...
saved_pool = bool(
    len(
        glob.glob(
            os.path.join(pool_path, "pool*{:d}layers.pt".format(args.pooling_layers))
        )
    )
)
